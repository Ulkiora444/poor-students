...
@Component({
  selector: 'app-save-home-section',
  templateUrl: './save-home-section.component.html',
  styleUrl: './save-home-section.component.css'
})
export class SaveHomeSectionComponent {

  indexCheckSave: number = 0;
  ...
  changeStudent(index: number){
    ...
    console.log(this.changedStudents)
  }

  getSave(){
    if(this.indexCheckSave==0){
      let mas = {
        year: this.year,
        month: this.month,
      };
      this.appService.getSave(mas).subscribe((data: any) => {
        if(data.success){
          this.changedStudents = [];
        }
      })
    }
    else if(this.indexCheckSave==1 || this.indexCheckSave==2){
      let mas = {
        year: this.year,
        month: this.month,
      ...
      this.appService.getCheckSave(mas).subscribe((data: any) => {
        if(data.success){
          this.changedStudents = [];
          this.index_students = '';
        }
